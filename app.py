...
def save_pdf():
    try:
        ...
        for edit in edits:
            ...
            page.draw_rect(
                fitz.Rect(x0 - 1, y0 - 1, x1 + 10, y1 + 1),
                color=(1, 1, 1),
                fill=(1, 1, 1)
            )
            ...
            try:
                page.insert_text(
                    ...
                    edit['text'],
                    fontsize=size * 0.85,
                    fontname=edit.get('font', fontname),
                    color=tuple(color)
                )
            except Exception:
                page.insert_text(
        ...
        for img_data in images:
            ...
            ix = img_data['x'] * scale_x
            iy = img_data['y'] * scale_y
            iw = img_data['w'] * scale_x
            ih = img_data['h'] * scale_y
            ...
            page.insert_image(
                ...
                stream=base64.b64decode(img_src)
            )
        ...
        for tb in text_boxes:
            ...
            if len(ch) == 6:
                tr = int(ch[0:2], 16) / 255
                tg = int(ch[2:4], 16) / 255
                tb_b = int(ch[4:6], 16) / 255
            else:
                tr, tg, tb_b = 0, 0, 0
            ...
            if bg and bg != 'transparent':
                ...
                if len(bh) == 6:
                    br = int(bh[0:2], 16) / 255
                    bg_g = int(bh[2:4], 16) / 255
                    bb = int(bh[4:6], 16) / 255
        ...
        for sh in shapes:
            ...
            if shape_type == 'rectangle':
                ...
                if rotate_deg == 0:
                    ...
                    corners = [
                        rotate_point(sx, sy, cx, cy, rad),
                        rotate_point(sx + sw, sy, cx, cy, rad),
                        rotate_point(sx + sw, sy + sh_h, cx, cy, rad),
                        rotate_point(sx, sy + sh_h, cx, cy, rad)
                    ]
                ...
                if rotate_deg == 0:
                    ...
                    pts = [
                        rotate_point(
                            cx + (sw / 2) * math.cos(math.radians(i * 10)),
                            cy + (sh_h / 2) * math.sin(math.radians(i * 10)),
                            cx,
                            cy,
                            rad
                        )
                ...
                p1 = (
                    rotate_point(sx, sy, cx, cy, rad)
                    if rotate_deg != 0
                    else fitz.Point(sx, sy)
                )

                p2 = (
                    rotate_point(sx + sw, sy + sh_h, cx, cy, rad)
                    if rotate_deg != 0
                    else fitz.Point(sx + sw, sy + sh_h)
                )
                ...
                if shape_type == 'arrow':
                    ...
                    page.draw_line(
                        p2,
                        fitz.Point(
                            p2.x - aw * math.cos(ang - 0.4),
                            p2.y - aw * math.sin(ang - 0.4)
                        ),
                        color=(r, g, b),
                        width=stroke
                    )
...
def delete_page():
    try:
        ...
        return jsonify({
            'pdfBase64': base64.b64encode(output.read()).decode(),
